...
def main():
    ...
    if not args.config_file:
        ...
        logger.info('Usage: python lims_downloader.py /path/to/configuration.sample')
        sys.exit(1)
    ...
    for a_run in run_list:
        ...
        for a_lane in lane_info:
            ...
            if case == 3:
                logger.info(f'Deleting records in database for re-processed data ({run_info["run_name"]}, {a_lane[0]})')
                lims_database.delete_old_run(run_info, a_lane)
            
            if case != 1:
                logger.info(f'downloading new/re-processed data ({run_info["run_name"]}, {a_lane[0]})')
