...
class my_sqlite_db:
   def initial_sqlite(self,db_name):
     # print(db_name) 
      if os.path.isfile(db_name) == False:
         ...
         c.execute("CREATE TABLE data_files (file_ID INT, package_ID INT, new_name TEXT, original_name TEXT, file_size REAL, sample_name TEXT, biomaterial TEXT, biomaterial_type TEXT, comments TEXT, principal_investigator TEXT, MID_tag TEXT, barcode TEXT, num_reads INT pct_reads_in_lane REAL, folder_name TEXT, SHA256 TEXT)")
   ...
   def insert_value_fileinfo(self, conn, all_file_info):
      for a_row in all_file_info:
         if len(a_row) == 9:
            cur = conn.cursor()
            cur.execute('INSERT INTO data_packages(sample_name,biomaterial,biomaterial_type, comments, principal_investigator, MID_tag, barcode, num_reads pct_reads_in_lane) VALUES (?,?,?,?,?,?,?,?,?)', a_row)
      conn.commit()         
