...
class WebParser:
    def __init__(self, login_url, runlist_url, username, password):
        """Log into the NRC-LIMS website
        Args: 
            login_url (str): link to the login page 
        ...
        try:
            ...
            logger.info('cannot log into the nrc-lims web site')
            sys.exit(1)
    ...
    def get_runlist(self, table_id, link_column, status_column):
        ...
        Returns:
            str: a list of links to the completed sequence runs
        """
        packages = []
        try:
    ...
    def get_text_acell(self,a_cell):
        """ Get the text in a cell of a table"""
        a_text = a_cell.findAll(text = True)
        a_text = [i for i in a_text if i not in ("\n", "")]
        if len(a_text) == 0:
            a_text = ""
    ...
    def download_zipfile(self, urlAddress, outputFileName):
        ...
        Args:
            urlAddress (str): link to the file
            outputFileName (str): path and file name to hold the file
        Returns:
