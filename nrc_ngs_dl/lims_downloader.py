...
def main():
    ...
    try:
        ...
        logger.info('Usage: python lims_downloader.py -c /path/to/configuation.sample')
        sys.exit(1)
    ...
    if os.path.exists(DESTINATION_FOLDER) == False:
        logger.info('DESTINATION_FOLDER not exist')
        sys.exit(1)
        
    if os.access(DESTINATION_FOLDER, os.R_OK) == False:
        logger.info('Do not have permission to access the DESTINATION_FOLDER' )
        sys.exit(1)
    ...
    for a_run in run_list:
        ...
        for a_lane in lane_info:
            ...
            if case ==3:
                logger.info('Delete records in database for re-processed data (run_name %s, lane_index %s)' % (run_info['run_name'],a_lane[0]))
                lims_database.delete_old_run(run_info, a_lane)
            
            if case != 1:
                logger.info('Download new/re-processed data (run_name %s, lane_index %s)' % (run_info['run_name'],a_lane[0]))
