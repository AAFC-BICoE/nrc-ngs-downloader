...
class LimsDatabase:
    ...
    def __init__(self, db_name):
        ...
        if not os.path.isfile(db_name):
            try:
                ...
                logger.error(f'Cannot access the database {db_name}')
                raise 
            c = conn.cursor()
            ...
            try:
                ...
                logger.error('Cannot access the database {db_name}')
                raise
    ...
    def insert_action_info(self, action_info):
        ...
        self.conn.commit()
        logger.debug(f'insert_action_info command {command_str} {column_value}')
        return rowid

    def insert_end_time(self, action_id, end_time):
        """Insert the end time of the program execution into Sqlite database"""
        cur = self.conn.cursor()
        command_str = 'UPDATE application_action SET end_time ="' + end_time + '" WHERE action_id = ?'
        cur.execute(command_str, (action_id,))
        logger.debug(f'insert_end_time command {command_str}, {action_id}')
        self.conn.commit() 
    ...
    def update_package_downloaded(self, package_downloaded, action_id):
        ...
        logger.debug(f'update_package_downloaded command {command_str} {package_downloaded} {action_id}')
        self.conn.commit() 
    ...
    def insert_run_info(self, run_info, action_id):
        ...
        self.conn.commit()
        logger.debug('insert_run_info command {command_str} {column_value}')
        return rowid

    @staticmethod
    def validate_pair(all_pair, table_column):
    ...
    def get_run_case(self, a_run_info, a_lane_info):
        ...
        cur.execute('SELECT http_content_length FROM data_packages WHERE run_name =? and lane_index = ?',
                    (run_name, lane_index,))
    ...
    def delete_old_run(self, a_run_info, a_lane_info):
        ...
        run_name = a_run_info['run_name']
        lane_index = a_lane_info['lane_index']
        cur.execute('SELECT package_id FROM data_packages WHERE run_name =? and lane_index = ?',
                    (run_name, lane_index,))
        all_rows = cur.fetchall()
        if len(all_rows) == 1:
